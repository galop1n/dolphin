...
void PSTextureEncoder::Init()
{
	...
	sprintf(cache_filename, "%sdx11-PSENCODER-cs.cache", File::GetUserPath(D_SHADERCACHE_IDX).c_str());
	...
	bool isIntensity, bool scaleByHalf)
{
...
#endif
	{
		...
		ID3D11Buffer* encBuf[] = { m_encodeParams.get() };
		D3D::context->CSSetConstantBuffers(0, 1, encBuf);

		ID3D11UnorderedAccessView* encUav = m_outUav.get();
		D3D::context->CSSetUnorderedAccessViews(0, 1, &encUav);
		...
		ID3D11SamplerState* encSamp[] = { m_efbSampler.get() };
		D3D::context->CSSetSamplers(0, 1, encSamp);
	...
	bool isIntensity, bool scaleByHalf)
{
	...
	ComboKey key = MakeComboKey(dstFormat, srcFormat, isIntensity, scaleByHalf, g_ActiveConfig.backend_info.bSupportsShaderModel5);
	...
	if (it != m_staticShaders.end()) {
		...
		D3D_SHADER_MACRO macros[] = {
			...
			{ "SHADER_MODEL", g_Config.backend_info.bSupportsShaderModel5 ? "5" : "4" },
			{ nullptr, nullptr }
		};
		...
		m_shaderCache.Append(key, bytecode.Data(), bytecode.Size());
		shader = InsertShader(key, bytecode.Data(), bytecode.Size());
	}
...
ID3D11ComputeShader* PSTextureEncoder::InsertShader( ComboKey const &key, u8 const *data, u32 sz) {
...
bool PSTextureEncoder::InitDynamicMode()
{
	...
	hr = PD3DReflect(bytecode.Data(), bytecode.Size(), IID_ID3D11ShaderReflection, ToAddr(reflect));
