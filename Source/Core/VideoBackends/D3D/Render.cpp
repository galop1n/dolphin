...
Renderer::Renderer()
{
	...
	// Do we need to enable multisampling here? [galop1n] No the AA parameters only apply to line drawing,
	// We can ignore it as we render line with triangles.
...
void Renderer::SwapImpl(u32 xfbAddr, u32 fbWidth, u32 fbHeight,const EFBRectangle& rc,float Gamma)
{
	...
	{
		std::string fps = StringFromFormat("%.2fms - %d\n", s_fps ? 1000.f / float( s_fps ) : 0.f, s_fps);
	...
	for (unsigned int stage = 0; stage < 8; stage++) {
		if ( (1u<<stage)&mask ) {
			if (g_ActiveConfig.iMaxAnisotropy > 0 )
				...
				auto st = D3D::GetSamplerState(gx_state.sampdc[stage],"sampler state used to emulate the GX pipeline");
				D3D::context->PSSetSamplers(stage, 1, &st);
		}
	}
